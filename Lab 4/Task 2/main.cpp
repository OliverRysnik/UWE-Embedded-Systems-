...
//Temperature range the potentiometer maps across (20C - 100C)
...
//Maps potentiometer 0-100% to a temperature threshold
...
int main()
{
    ...
    while (true) {
        ...
        //Threshold from potentiometer position
...
//Maps the 0-100% pot value to a threshold between a minimum and a maxiumum
float calculateThreshold(float potPercent)
{
    return TEMP_THRESHOLD_MIN + (potPercent / 100.0f) * (TEMP_THRESHOLD_MAX - TEMP_THRESHOLD_MIN);
}

//Threshold parameter added so it prints alongside readings
void printReadings(float tempC, float potPercent, bool gasDetected, float threshold)
{
    char buffer[100];
    uartUsb.write("-----\r\n", 7);
