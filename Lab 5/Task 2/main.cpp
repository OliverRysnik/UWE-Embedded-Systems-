...
int main() {
    ...
    while (true) {
        ...
        if (temperatureC > threshold || gasDetected) {
            if (!alarmActive) {
        ...
        if (liveFeedEnabled) {
            ...
            if (liveFeedAccumulator >= LIVE_FEED_DELAY_MS) {
                ...
                int len = snprintf(buffer, sizeof(buffer), 
                    "\r\n--- LIVE FEED ---\r\nTemp: %.2f C | Thresh: %.2f C | Gas: %s | Alarm: %s\r\n", 
                    temperatureC, threshold, gasDetected ? "DETECTED" : "Clear", alarmActive ? "ACTIVE" : "OFF");
...
void uartTask() {
    ...
    if (uartUsb.readable()) {
        ...
        if (receivedChar == 'l' || receivedChar == 'L') {
            ...
            if (liveFeedEnabled) {
                uartUsb.write("\r\nLive Feed: ENABLED (Updating every 2s)\r\n", 41);
            } else {
                uartUsb.write("\r\nLive Feed: DISABLED\r\n", 22);
            }
